...
const client = new GManClient({
  ...
  autoLaunch: true,            // Автоматически запускать демон g-mand, если он не запущен
  daemonPath: './bin/g-mand'   // Опционально: путь к бинарнику g-mand
});
```
...
# Автоматически импортировать доступы и секреты из текущей папки (.env / config.json)
...
## Безопасность и изоляция сессий

В целях обеспечения максимальной безопасности Steam-аккаунтов, **конфиденциальные токены сессий (куки) и секреты (shared secrets) никогда не передаются по gRPC / IPC каналам** обратно в JS-клиент.
* Все cookies авторизованной веб-сессии хранятся строго в оперативной памяти демона `g-mand`.
* JS-эмуляторы (`steamcommunity` и `tradeoffer-manager`) перенаправляют свои HTTP-запросы в демон через RPC-метод `ExecRequest`.
* Демон на лету автоматически подставляет правильные куки сессии и актуальный `sessionid` в каждый уходящий запрос. Это сохраняет полную обратную совместимость со сторонними библиотеками (такими как `tf2autobot`), исключая риск утечки сессионных данных.

Если вам действительно нужно напрямую обратиться к IPC-каналу демона, вы можете:
1. Выполнить тестовый GET-запрос через `client.execRequest` на любую страницу Steam (например, `/my`).
2. В возвращаемых HTTP-заголовках перехватить новые `Set-Cookie`, которые присылает Steam, и распарсить их.
