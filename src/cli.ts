...
function printUsage() {
  console.log(`${ColorCyan}${ColorBold}G-MAN Command Line Control Interface (gmanctl-node)${ColorReset}\n`);
  console.log('Usage:');
  console.log('  gmanctl-node [command] [args...]');
  console.log('\nSystem Commands:');
  console.log(`  ${'status'.padEnd(30)} Show daemon status, Steam connection and resource metrics`);
  console.log(`  ${'stop'.padEnd(30)} Stop the background daemon gracefully`);
  console.log(`  ${'gc'.padEnd(30)} Force manual garbage collection and free physical memory`);
  console.log(`  ${'events'.padEnd(30)} Stream real-time daemon and game coordinator events`);
  console.log('\nGame Commands:');
  console.log(`  ${'play <appid>'.padEnd(30)} Launch game session & initialize Game Coordinator`);
  console.log(`  ${'exit-game'.padEnd(30)} Close active game session, return to simple online mode`);
  console.log('\nAction Commands:');
  console.log(`  ${'exec <appid> <action> [params]'.padEnd(30)} Execute game action (e.g., exec 440 craft-metal)`);
  console.log(`  ${'exec <appid> inventory'.padEnd(30)} Quick shortcut to query game backpack items`);
  console.log(`  ${'update-prices <entry> [entry...]'.padEnd(30)} Update manual pricing database entries`);
  console.log('\nGuard Commands:');
  console.log(`  ${'guard status'.padEnd(30)} Show Steam Guard configuration status`);
  console.log(`  ${'guard code'.padEnd(30)} Generate current Steam Guard 2FA TOTP code`);
  console.log(`  ${'guard list'.padEnd(30)} List pending Steam Guard confirmations`);
  console.log(`  ${'guard respond <id> <accept|decline>'.padEnd(30)} Accept or decline a confirmation`);
  console.log(`  ${'guard import <shared> <identity> <device> [name] [token]'.padEnd(30)} Import Steam Guard secrets`);
  console.log('\nGlobal Parameters:');
  console.log("  Arguments for 'exec' actions can be passed in key=value format (e.g., type=scrap).");
  console.log('  Arguments for \'update-prices\' are formatted as: sku=buy_keys,buy_metal,sell_keys,sell_metal');
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      case 'play':
        if (args.length < 2) {
          console.error(`${ColorRed}Error: 'play' command requires an AppID. Example: gmanctl-node play 440${ColorReset}`);
          process.exit(1);
        }
        const playAppID = parseInt(args[1], 10);
        if (isNaN(playAppID)) {
          console.error(`${ColorRed}Error: Invalid AppID "${args[1]}". Must be an integer.${ColorReset}`);
          process.exit(1);
        }
      ...
      case 'exec':
        ...
        if (isNaN(execAppID)) {
          console.error(`${ColorRed}Error: Invalid AppID "${args[1]}".${ColorReset}`);
          process.exit(1);
        }
        const action = args[2];
      ...
      case 'guard':
        ...
        switch (sub) {
          ...
          case 'respond':
            ...
            const confID = args[2];
            const accept = args[3] === 'accept' || args[3] === 'true' || args[3] === '1';
          ...
          case 'import':
            ...
            const token = args[6] || '';
            await handleGuardImport(client, shared, identity, device, account, token);
            break;
          default:
            console.error(`${ColorRed}Unknown guard subcommand: ${sub}${ColorReset}`);
            process.exit(1);
        }
        break;
      default:
    ...
    console.error(`${ColorRed}Unexpected client error: ${err.message}${ColorReset}`);
    process.exit(1);
  } finally {
    if (command !== 'events') {
      client.close();
...
async function handleStatus(client: GManClient) {
  ...
  console.log(`${ColorBold}${ColorCyan}=== G-MAN DAEMON STATUS ===${ColorReset}`);
  ...
  console.log(`Memory Usage:     ${ColorYellow}${memMB.toFixed(2)} MB${ColorReset}`);
}
...
async function handleFreeMemory(client: GManClient) {
  console.log(`${ColorYellow}Triggering manual Garbage Collection in daemon...${ColorReset}`);
  const resp = await client.freeMemory();
  console.log(`${ColorGreen}${resp.message}${ColorReset}`);
  const memMB = Number(resp.memory_bytes) / 1024.0 / 1024.0;
  console.log(`New Memory Usage: ${ColorYellow}${memMB.toFixed(2)} MB${ColorReset}`);
}

function handleStreamEvents(client: GManClient) {
  console.log(`${ColorCyan}Streaming real-time daemon events... (Press Ctrl+C to exit)${ColorReset}\n`);
  ...
  stream.on('data', (resp) => {
    ...
    console.log(`[${timeStr}] ${ColorGreen}${evType}${ColorReset}: ${resp.payload_json}`);
  });
  
  stream.on('error', (err: any) => {
    console.error(`${ColorRed}Stream connection lost: ${err.message}${ColorReset}`);
    client.close();
    process.exit(1);
  });
...
async function handlePlay(client: GManClient, appID: number) {
  console.log(`${ColorCyan}Launching session for game AppID ${appID}...${ColorReset}`);
  const resp = await client.playGame(appID);
  console.log(`${ColorGreen}${resp.message}${ColorReset}`);
}

async function handleExitGame(client: GManClient) {
  console.log(`${ColorCyan}Stopping playing session and exiting game...${ColorReset}`);
  const resp = await client.exitGame();
  console.log(`${ColorGreen}${resp.message}${ColorReset}`);
}

async function handleExec(client: GManClient, appID: number, action: string, params: Record<string, string>) {
  console.log(
...
async function handleUpdatePrices(client: GManClient, priceArgs: string[]) {
  ...
  for (const arg of priceArgs) {
    ...
    if (isNaN(buyKeys) || isNaN(buyMetal) || isNaN(sellKeys) || isNaN(sellMetal)) {
      console.error(`${ColorRed}Error: Invalid price number formatting in "${arg}"${ColorReset}`);
      process.exit(1);
    }
...
async function handleGuardStatus(client: GManClient) {
  ...
  console.log(`${ColorBold}${ColorCyan}=== STEAM GUARD STATUS ===${ColorReset}`);
...
async function handleGuardCode(client: GManClient) {
  ...
  console.log(`${ColorBold}${ColorGreen}Current Steam Guard Code: ${resp.code}${ColorReset}`);
}
...
async function handleGuardList(client: GManClient) {
  ...
  console.log(`${ColorBold}${ColorCyan}=== PENDING CONFIRMATIONS (${confs.length}) ===${ColorReset}`);
  
  for (let i = 0; i < confs.length; i++) {
    const c = confs[i];
    console.log(`\n${ColorBold}[${i + 1}]${ColorReset} ${ColorGreen}${c.title}${ColorReset}`);
    console.log(`    ID: ${ColorYellow}${c.id}${ColorReset}  Type: ${ColorYellow}${c.type_name}${ColorReset}`);
    if (c.description) {
  ...
  console.log(`${ColorGray}To decline: gmanctl-node guard respond <id> decline${ColorReset}`);
}

async function handleGuardRespond(client: GManClient, confirmationId: string, accept: boolean) {
  const action = accept ? 'accepting' : 'declining';
  console.log(`${ColorCyan}${action} confirmation ${confirmationId}...${ColorReset}`);
...
async function handleGuardImport(
  ...
  const resp = await client.guardImport(sharedSecret, identitySecret, deviceId, accountName, refreshToken);
  if (resp.success) {
    console.log(`${ColorGreen}${resp.message}${ColorReset}`);
  } else {
    console.log(`${ColorRed}Failed: ${resp.message}${ColorReset}`);
